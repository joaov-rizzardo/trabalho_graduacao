...
export default async function signIn(req: Request, res: Response) {
  try {
    const body = req.body;
    const authenticatedResult = await authenticateUser(
      body.username,
      body.password
    );
    if (
      authenticatedResult.ok === false ||
      authenticatedResult.user === false
    ) {
      return res
        .status(400)
        .send(default400Response(authenticatedResult.errors));
    }
...
export async function authenticateUser(
  username: string,
  password: string
): Promise<{ ok: boolean; errors: string[]; user: UserType | false }> {
  const userDAO = new UserDAO();
  const userData = await userDAO.getUserByField("binary username", username);
  if (userData === false) {
