...
export default class BillDAO {
  ...
  public async getInstallmentsByBillId(billId: number) {
    const response = (await query(
      `SELECT * FROM BillInstallments WHERE billId = ?`,
      [billId]
    )) as [InstallmentFieldsType[], FieldPacket[]];
    ...
    return installmentData.map((installment) => {
      return {
        ...
        payedAt:
          installment.payedAt !== null
            ? convertDateObjectDatetimeToString(installment.payedAt)
            : undefined,
  ...
  public async getBillById(billId: number) {
    const response = (await query(`SELECT * FROM UserBills WHERE billId = ?`, [
      billId,
    ])) as [BillFieldsType[], FieldPacket[]];
    ...
    return {
      ...
      canceledAt:
        billData.canceledAt !== null
          ? convertDateObjectDatetimeToString(billData.canceledAt)
          : undefined,
    };
  }
  ...
  public async getInstallmentById(installmentId: number) {
    const response = (await query(
      `SELECT * FROM BillInstallments WHERE installmentId = ?`,
      [installmentId]
    )) as [InstallmentFieldsType[], FieldPacket[]];
    const installmentData = response[0][0];
    if (!installmentData) {
      throw new Error(
        `Nenhuma parcela foi encontrada para o ID: ${installmentId}`
      );
    }
    return {
      ...
      payedAt:
        installmentData.payedAt !== null
          ? convertDateObjectDatetimeToString(installmentData.payedAt)
          : undefined,
  ...
  public async getNotCanceledBillsByUserId(userId: number) {
    const response = (await query(
      `SELECT * FROM UserBills WHERE userId = ? AND isCanceled = 0`,
      [userId]
    )) as [BillFieldsType[], FieldPacket[]];
      ...
      return {
        ...
        canceledAt:
          bill.canceledAt !== null
            ? convertDateObjectDatetimeToString(bill.canceledAt)
            : undefined,
      };
    });
  }

  public async getBillInstallmentsByFilters({
    userId,
    startDate,
    finishDate,
    payed,
  }: GetBillInstallmentsByFiltersParamsType) {
    ...
    const response = (await query(sql, params)) as [
      GetBillInstallmentsByFiltersReturnType[],
      FieldPacket[]
    ];
    const installmentData = response[0];
    return installmentData.map((installment) => {
