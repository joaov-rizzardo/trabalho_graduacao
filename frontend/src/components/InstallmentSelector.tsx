...
export interface EarningCategorySelectorProps {
  selectedInstallment: GetInstallmentsType | null;
  setInstallment: React.Dispatch<
    React.SetStateAction<null | GetInstallmentsType>
  >;
}

export interface InstallmentSelectorRef {
  removeInstallment: (id: number) => void;
}
...
const InstallmentSelector = forwardRef<
  InstallmentSelectorRef,
  EarningCategorySelectorProps
>(
  (
    { selectedInstallment, setInstallment }: EarningCategorySelectorProps,
    ref
  ) => {
    ...
    const removeInstallment = (id: number) => {
      setInstallments((installments) =>
        installments.filter((installment) => installment.installmentId !== id)
      );
    };
    return (
      <>
        ...
        >
          ...
          <Text style={styles.inputText}>
            ...
            {selectedInstallment !== null
              ? `${selectedInstallment.description} ${selectedInstallment.installmentNumber}ª parcela`
              : "Selecione uma conta"}
          </Text>
          ...
          >
            ...
            {installments.flatMap((installment, index) => {
              ...
              return (
                ...
                >
                  <Image
                    source={SpendingCategoryImages[installment.categoryKey]}
                    style={styles.itemImage}
                  />
                  <View style={styles.itemTextContainer}>
                    <Text
                      style={{ ...styles.itemText, ...styles.descriptionText }}
                    >
                      {installment.description}
                    </Text>
                    <Text
                      style={{ ...styles.itemText, ...styles.categoryText }}
                    >
                      {installment.categoryDescription}
                    </Text>
                  </View>
                  <View>
                    <Text style={{ ...styles.itemText, ...styles.valueText }}>
                      {installment.value?.toLocaleString("pt-BR", {
                        style: "currency",
                        currency: "BRL",
                      })}
                    </Text>
                    <Text style={{ ...styles.itemText, ...styles.dueDateText }}>
                      {formatDateToBrString(new Date(installment.dueDate))}
                    </Text>
                  </View>
