...
export default function InvestGoals() {
    ...
    const {finances, updateFinances, updateLevel, level} = useContext(UserContext)
    ...
    async function handleInvestGoal(){
        ...
        const {data: {rewards, userFinance, userLevel}} = await backendApi.put<InvestGoalType>('/goal/invest', {
            goalId: goal?.goalId,
    ...
    function handleValidations(){
        ...
        if(goal === null){
            return {ok: false, message: 'Por favor, selecione uma meta para realizar o investimento'}
        }
        if(floatValue <= 0){
            return {ok: false, message: 'O valor de investimento deve ser superior a zero'}
        }
        if(floatValue > (goal.value - goal.progressValue)){
            return {ok: false, message: 'O valor informado é maior do que a quantia pendente para a meta'}
        }
    ...
    return (
        <ScreenTemplate>
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.container}>
                <GoalSelector selectedGoal={goal} setGoal={setGoal} />
                <View style={styles.badgesContainer}>
                    <View style={styles.badgeItem}>
                        <Image style={styles.badgeImage} source={require('../../assets/images/dinheiro.png')} />
                        <View style={styles.badgeTextContainer}>
                            <Text style={styles.badgeTitle}>Valor restante</Text>
                            <Text style={styles.badgeValue}>{(goal !== null ? (goal?.value - goal?.progressValue) : 0).toLocaleString('pt-BR', {
                                style: 'currency',
                                currency: 'BRL',
                            })}</Text>
                        </View>
                    ...
                    <View style={styles.badgeItem}>
                        <Image style={styles.badgeImage} source={require('../../assets/images/moedas.png')} />
                        <View style={styles.badgeTextContainer}>
                            <Text style={styles.badgeTitle}>Saldo restante</Text>
                            <Text style={styles.badgeValue}>{finances.balance.toLocaleString('pt-BR', {
                                style: 'currency',
                                currency: 'BRL',
                            })}</Text>
                        </View>
                    ...
                    <CustomInput.Input
                        ...
                        onChangeText={text => {
                            if(goal === null){
                                ...
                                setInvestmentValue(floatValue > pendingValue ? pendingValue.toString() : maskedValue)
                            }
                        }}
                    />
                </CustomInput.Container>
                <CustomButton text="Realizar investimento" onPress={async () => {
                    setIsInvesting(true)
