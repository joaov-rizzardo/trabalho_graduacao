...
import { BillType, CancelBillType } from "../types/ApiResponses/BillTypes";
import { UserContext } from "../contexts/UserContext";

async function findActiveBills() {
  try {
    const { data: bills } = await backendApi.get<BillType[]>(
      "/bill/activeBills"
    );
    return bills.sort(
      (a, b) =>
...
export default function ManageBills() {
  ...
  async function handleCancelBill(billId: number) {
    try {
      const {
        data: { userFinance },
      } = await backendApi.put<CancelBillType>(`bill/cancel/${billId}`);
      ...
      return openAlertPopup({
        title: "Atenção",
        content:
          "Não foi possível cancelar a conta, tente novamente mais tarde.",
      });
    }
  }
        ...
        {bills.flatMap((bill, index) => {
          ...
          return (
            <View style={styles.itemContainer} key={index}>
              <Image
                source={SpendingCategoryImages[bill.categoryKey]}
              ...
              <View style={{ alignSelf: "flex-end" }}>
                ...
                >
                  {bill.installments[0].value?.toLocaleString("pt-BR", {
                    style: "currency",
                    currency: "BRL",
                  })}
                </Text>
              ...
              {(bill.billType === "F" ||
                dateDiferenceInDays(new Date(bill.createdAt), new Date()) <
                  1) && (
                <IconButton
                  onPress={() => {
                    openConfirmPopUp({
                      title: "Atenção",
                      content:
                        "Você tem certeza que deseja cancelar a conta? Essa ação é irreversível",
