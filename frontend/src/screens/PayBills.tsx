...
import { GetInstallmentsType, PayInstallmentType } from "../types/ApiResponses/BillTypes";
...
export default function PayBills() {
    ...
    const {openConfirmPopUp, openAlertPopup, openModalRewards, openModalLevelup} = useContext(PopupContext)
    ...
    async function handleBillPay(){
        ...
        if(parseFloat(paymentValue) !== installment?.value){
            openConfirmPopUp({
                title: 'Atenção',
                content: 'Você informou um valor diferente do cadastrado na conta, deseja prosseguir mesmo assim?',
                onConfirm: () => {
                    payBill()
                }
            })
    ...
    async function payBill(){
        try {
            setIsPaying(true)
            const {data: {rewards, userFinance, userLevel}} = await backendApi.put<PayInstallmentType>('/bill/installment/pay', {
            ...
            console.log(error)
            openAlertPopup({
                content: 'Não foi possível realizar o pagamento da conta, tente novamente mais tarde',
            })
        }finally {
            setIsPaying(false)
        }
    ...
    function handleValidations(){
        ...
        if(installment === null){
            return {ok: false, message: 'Por favor, selecione uma conta para realizar o pagamento'}
        }
        ...
        if(paymentMethod === "EC" && finances.currentSavings < floatPaymentValue){
            return {ok: false, message: 'Economias insuficientes para realizar o pagamento da conta'}
        }
        if(paymentMethod === "SA" && finances.balance < floatPaymentValue){
            return {ok: false, message: 'Saldo insuficiente para realizar o pagamento da conta'}
        }
        ...
        <ScreenTemplate>
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.container}>
                ...
                <CustomInput.Container>
                    ...
                    <CustomInput.Input keyboardType="number-pad" placeholder="Valor pago" value={paymentValue} onChangeText={text => setPaymentValue(moneyMask(text))}/>
                </CustomInput.Container>
                <PaymentMethodSelector paymentMethod={paymentMethod} setPaymentMethod={setPaymentMethod}/>
                <CustomButton text="Pagar conta" onPress={handleBillPay} loading={isPaying}/>
            </ScrollView>
        </ScreenTemplate>
    )
}
