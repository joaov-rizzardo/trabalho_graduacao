...
import { CheckEmailType, CheckUserType, SignUpType } from "../types/ApiResponses/AuthenticationTypes";
...
export default function Register({ navigation }: RegisterProps) {
    ...
    async function handleRegister(){
        try {
            ...
            if(loginResponse.ok === false){
                openAlertPopup({
                    title: 'Sucesso',
                    content: 'Sua conta foi criada com sucesso. Realize o login para desfrutar das funcionalidades do nosso app',
            ...
            openAlertPopup({content: 'Ocorreu um erro inesperado, por favor tente novamente mais tarde.'})
        }finally{
    ...
    async function signUp(){
        try{
            ...
            return {ok: false, message: 'Não foi possível realizar o cadastro. Tente novamente mais tarde'}
        }
    }

    async function validateFields(): Promise<{ok: boolean, message: string}>{
        const fieldValues = Object.values(registerForm)
        if(fieldValues.includes('')){
            return {ok: false, message: 'Por favor, informe todos os campos para realizar o cadastro.'}
        }
        if(registerForm.password.length < 8){
            return {ok: false, message: 'A senha deve conter no minimo 8 caracteres'}
        }
        ...
        if(!await verifyEmailDisponibility()){
            return {ok: false, message: 'O email informado já está sendo utilizado.'}
        }
        if(!await verifyUserDisponibility()){
            return {ok: false, message: 'O usuário informado já está sendo utilizado.'}
        }
        return {ok: true, message: ''}
    }
    ...
    async function verifyEmailDisponibility(){
        try{
            const response = await backendApi.post<CheckEmailType>('/authentication/checkEmail', {
                email: registerForm.email
            })
    ...
    async function verifyUserDisponibility(){
        try{
            const response = await backendApi.post<CheckUserType>('/authentication/checkUser', {
                username: registerForm.username
            })
            const {isAvailable} = response.data
            return isAvailable
            ...
            <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
                <CustomInput.Container>
                    ...
                    <CustomInput.Input
                        ...
                        onChangeText={text => setRegisterForm(prevState => {
                            return {
                                ...prevState,
                                username: text
                            }
