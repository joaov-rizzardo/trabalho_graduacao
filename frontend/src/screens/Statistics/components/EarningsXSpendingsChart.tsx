...
async function findTransactions(startsAt: string, endsAt: string) {
  try {
    const { data: transactions } = await backendApi.post<GetTransactionsType>(
      "/management/getTransactions",
      {
        startDate: startsAt,
        finishDate: endsAt,
      }
...
function acumulateTransactions({ spendings, earnings }: GetTransactionsType) {
  spendings.sort(
    (a, b) =>
      new Date(b.spendingDate).getTime() - new Date(a.spendingDate).getTime()
  );
  earnings.sort(
...
export default function EarningsXSpendingsChart({
  interval,
}: EarningsXSpendingsChartProps) {
  const [transactions, setTransactions] = useState<GetTransactionsType | null>(
    null
  );
  ...
  const transactionsDates = useMemo(() => {
    ...
    const dates = [
      ...
      ...transactions.spendings.map(
        (spending) => new Date(spending.spendingDate)
      ),
    ];
      ...
      {acumulatedTransactions !== null && (
        <LineChart
          ...
          data={{
            ...
            datasets: [
              {
                data: acumulatedTransactions.earnings.map(
                  (earning) => earning.value
                ),
                color: () => "#7FD7B2",
              },
              {
                data: acumulatedTransactions.spendings.map(
